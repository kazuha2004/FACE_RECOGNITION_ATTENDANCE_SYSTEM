...
from chatbot2 import ChatBot  # Make sure this imports the ChatBot class
...
class Face_Recognition_System:
    def __init__(self, root):
        ...
        b3 = Button(self.root, image=self.photoimg10, cursor="hand2", bg="black",command=self.Report_data)
        b3.place(x=800, y=480, width=220, height=220)

        b3_1 = Button(self.root, text="Report and Analytics",command=self.Report_data ,cursor="hand2", font=("Helvetica", 15, "bold"), bg="black", fg="white")
        ...
        b3 = Button(self.root, image=self.photoimg11, cursor="hand2", bg="black", command=self.root.quit)  # Close the application
        b3.place(x=1100, y=480, width=220, height=220)

        b3_1 = Button(self.root, text="Exit", cursor="hand2", font=("Helvetica", 15, "bold"), bg="black", fg="white", command=self.root.quit)
    ...
    def toggle_chat_bot(self):
          if self.chatbot_open:
                self.chatbot_window.destroy()  # Close the chatbot window
                self.chatbot_open = False
          else:
                self.chatbot_window = Toplevel(self.root)  # Open the chatbot window
                self.app = ChatBot(self.chatbot_window)
                self.chatbot_open = True # Launch the ChatBot in a new window
